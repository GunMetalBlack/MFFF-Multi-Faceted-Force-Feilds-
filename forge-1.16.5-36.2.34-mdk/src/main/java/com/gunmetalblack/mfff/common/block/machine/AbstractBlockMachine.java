...
public abstract class AbstractBlockMachine extends Block implements IWaterLoggable{
     ...
     * For example, at the time of writing, it is also used by the (which is not a multiblock).
     */
     ...
     * that have more than a handful of possible values (ex. energy storage). Complex data should instead be handled using a {@link TileEntity} .
     */
    @Override
     ...
     * See Also (when using Minecraft's vanilla render pipeline)
     ...
     * blocks using <br>{@link net.minecraftforge.client.model.pipeline.BlockInfo#updateLightMatrix}.
     */
    @SuppressWarnings("deprecation")
    @Override
    @OnlyIn(Dist.CLIENT)
