...
public class TileMachine extends TileEntity implements INamedContainerProvider, INameable {
    ...
    private final MFFFEnergyStorage energyStorage;
    ...
    public TileMachine(TileEntityType<?> tileEntityTypeIn, Supplier<ContainerType<? extends AbstractContainerMachine>> containerType, int inventorySize, int forgeEnergyCapacity, int forgeEnergyPerTickIn, int forgeEnergyPerTickOut, ITextComponent defaultName) {
        ...
        this.energyStorage = new MFFFEnergyStorage().setCapacity(forgeEnergyCapacity, false).setMaxReceive(forgeEnergyPerTickIn).setMaxExtract(forgeEnergyPerTickOut).setCallbackOnChanged(this::setChanged);
    ...
    private ItemStackHandler createHandler(int inventorySize) {
        return new MFFFItemStackHandler(inventorySize, this::onInventorySlotChanged, this::isInventoryItemValid);
    }
    ...
    public void tickIngestEnergy() {
        ...
        energyStorageLazyOptional.ifPresent(energyStorageUncast -> {
            if(energyStorageUncast instanceof MFFFEnergyStorage) {
                MFFFEnergyStorage energyStorage = (MFFFEnergyStorage)energyStorageUncast;
    ...
    public int ingestEnergy(int maxAmount, Direction[] directions) {
        ...
        for(int i = 0; totalRequesting > 0 && i < directions.length; ++i) {
            ...
            if(blockEntity != null) {
                LazyOptional<IEnergyStorage> neighborCapOptional = blockEntity.getCapability(MFFF.FORGE_ENERGY_CAPABILITY, direction.getOpposite());
    ...
    public boolean tryConsumeEnergy(int energyToConsume) {
        MFFFEnergyStorage energyStorage = (MFFFEnergyStorage)energyStorageLazyOptional.orElse(null);
    ...
    public void updateNearbyClients() {
        if(level != null && !level.isClientSide()) {
            ...
            if (updatePacket != null && level.getServer() != null) {
                level.getServer().getPlayerList().broadcast(null, getBlockPos().getX(), getBlockPos().getY(), getBlockPos().getZ(), MFFF.distProxy.getTileEntityUpdateDistance(), level.dimension(), updatePacket);
            }
        }
    }
    ...
    public void onDataPacket(NetworkManager net, SUpdateTileEntityPacket pkt) {
        if(level != null && level.isClientSide()) {
            ...
            MFFF.distProxy.updateScreenMachine();
        }
    }
